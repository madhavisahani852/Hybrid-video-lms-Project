...
from src.storage import get_job, update_job
...
def _normalize_progress(
    ...
    update_job(job_id, progress=normalized_value)
...
def _resolve_checkpoint_path(model_name: str) -> str:
    ...
    raise PipelineError(
        "LatentSync inference checkpoint is not available. The repository expects "
        "the official LatentSync runtime and checkpoint file named "
        "`latentsync_unet.pt` plus Whisper model assets, configured through "
        "LATENTSYNC_CHECKPOINT_PATH, LATENTSYNC_WHISPER_PATH, and the official "
...
def _resolve_config_path() -> str:
    ...
    raise PipelineError(
        "LatentSync UNet config is missing. Configure LATENTSYNC_CONFIG_PATH "
        "or LATENTSYNC_CONFIG_DIR to point at the official repo config directory."
    )
...
def _resolve_scheduler_config_path() -> str:
    ...
    raise PipelineError(
        "LatentSync scheduler config is missing. Configure "
        "LATENTSYNC_SCHEDULER_CONFIG_PATH or LATENTSYNC_CONFIG_DIR "
        "to the official LatentSync config directory."
    )


def _merge_audio_chunks(audio_path: str) -> str:
    """Accept audio file/chunks and return a valid audio file."""
    raw = audio_path
    ...
    if isinstance(raw, (list, tuple)):
        ...
        list_path = (
            Path(
                tempfile.mkdtemp(
                    prefix="talking_head_audio_list_"
                )
            )
            / "chunks.txt"
        )
        ...
        with list_path.open("w", encoding="utf-8") as handle:
            for chunk in chunk_paths:
                handle.write(
                    f"file '{Path(chunk).resolve().as_posix()}'\n"
                )
    ...
    if path.is_dir():
        ...
        return _merge_audio_chunks(
            [str(p) for p in audio_candidates]
        )

    if not path.exists():
        raise PipelineError(
            f"Audio input file not found: '{audio_path}'"
        )

    return str(path)


def _make_reference_video(
    image_path: str,
    output_video_path: str,
) -> str:
    """Convert the still face image into a minimal LatentSync video input."""
    source = Path(image_path)

    if not source.exists() or not source.is_file():
        raise PipelineError(
            f"Face input image not found: '{image_path}'"
        )
    ...
    if frame is None:
        raise PipelineError(
            f"Face input image could not be decoded: '{image_path}'"
        )
    ...
    reference_fd, reference_path = tempfile.mkstemp(
        prefix="latentsync_ref_",
        suffix=".mp4",
    )

    os.close(reference_fd)

    Path(reference_path).parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    ...
    if not writer.isOpened():
        raise PipelineError(
            "OpenCV could not initialize a temporary MP4 reference video "
            "for LatentSync."
        )
...
class _OfficialLatentSyncBackend:
    ...
    def _load_pipeline(self):
        ...
        try:
            ...
            raise PipelineError(
                "The official LatentSync runtime is not installed. Install "
                f"the package from {LATENTSYNC_MODEL_REPO} with its required "
                "dependencies before inference."
            ) from exc

        if not Path(self.checkpoint_path).exists():
            raise PipelineError(
                f"LatentSync checkpoint not found at "
                f"'{self.checkpoint_path}'. Check "
                "LATENTSYNC_CHECKPOINT_PATH or MODEL_CACHE_DIR/latentsync."
            )

        if not Path(self.whisper_path).exists():
            raise PipelineError(
                f"LatentSync Whisper checkpoint not found at "
                f"'{self.whisper_path}'. Check "
                "LATENTSYNC_WHISPER_PATH or "
                "MODEL_CACHE_DIR/latentsync/checkpoints/whisper/."
            )

        if not Path(self.config_path).exists():
            raise PipelineError(
                f"LatentSync UNet config not found at "
                f"'{self.config_path}'. Check "
                "LATENTSYNC_CONFIG_PATH or LATENTSYNC_CONFIG_DIR."
            )

        if not Path(self.scheduler_config_path).exists():
            raise PipelineError(
                f"LatentSync scheduler config not found at "
                f"'{self.scheduler_config_path}'. Check "
                "LATENTSYNC_SCHEDULER_CONFIG_PATH or "
                "LATENTSYNC_CONFIG_DIR."
            )
        ...
        if (
            scheduler_source.is_file()
            and scheduler_source.suffix.lower() == ".json"
        ):
    ...
    def run(
        ...
        *,
        ...
        reference_video = _make_reference_video(
            image_path,
            output_path,
        )
        ...
        try:
            ...
            raise PipelineError(
                "LatentSync backend executed without producing a usable MP4 "
                f"output at the requested path '{output_path}'."
            )
...
def get_inference_backend(model_name: str):
    ...
    scheduler_config_path = (
        _resolve_scheduler_config_path()
    )
    ...
    try:
        ...
        raise PipelineError(
            "LatentSync inference engine is not installed in this environment. "
            "Install the official LatentSync package and configure "
            "LATENTSYNC_CHECKPOINT_PATH, LATENTSYNC_WHISPER_PATH, and "
            "LATENTSYNC_CONFIG_PATH before running the pipeline."
        ) from exc
    ...
    try:
        ...
        raise PipelineError(
            "The official LatentSync pipeline package is not installed or is "
            "incomplete. Install the repository from the official source and "
            "configure its model assets."
        ) from exc

    if not callable(LipsyncPipeline):
        raise PipelineError(
            "Official LatentSync pipeline is available but does not expose "
            "LipsyncPipeline."
        )
...
def run_talking_head_pipeline(
    ...
    """Execute a talking-head inference job with persistent SQLite job state."""
    logger.info(
    ...
    if job is None:
        logger.error(
            f"Pipeline error: job {job_id} not found in SQLite state store."
        )
        return
    ...
    output_target = _resolve_default_output_path(
        job_id,
        output_path,
    )
    ...
    try:
        resolved_image_path = _resolve_service_path(
            image_path
        )

        if (
            not resolved_image_path
            or not Path(resolved_image_path).exists()
        ):
            raise PipelineError(
                f"Input face image does not exist: '{image_path}'"
            )

        resolved_audio_path = _resolve_service_path(
            audio_path
        )

        audio_input = _merge_audio_chunks(
            resolved_audio_path
        )
        ...
        if not Path(backend_output).exists():
            raise PipelineError(
                f"Inference completed but no MP4 was generated at "
                f"'{backend_output}'."
            )

        if not validate_video_output(backend_output):
            raise PipelineError(
                f"Generated output '{backend_output}' is not a valid MP4 "
                "or failed validation."
            )

        output_file = Path(backend_output).resolve()

        if not output_file.is_file():
            raise PipelineError(
                f"Validated output file is no longer available at "
                f"'{output_file}'."
            )

        output_url = (
            f"/api/v1/outputs/"
            f"{job_id}/outputs/{output_file.name}"
        )
        ...
        logger.info(
            f"Pipeline completed successfully for job "
            f"{job_id}: {output_file}"
        )
        ...
        logger.error(
            f"Pipeline failed for job {job_id}: {exc}"
        )
